...
fast_nod_rate=np.pi /u.min
nod_params_Evan =   [0.2, slow_nod_rate]
nod_params_Sophia = [10,  slow_nod_rate]
# nod_params_Evan =   [2,  fast_nod_rate]
# nod_params_Sophia = [30, fast_nod_rate]

# same between all cases: CasA, 24 hrs of obs
experimental_setups=[ ["Evan",   "nod", False, nod_params_Evan   ],  # small-amplitude slow nod; realistic transits] 
                      ["Mike",   "el",  True,  None              ],  # 90º feed rotation <-> measure using both pols 
                      ["Sophia", "nod", False, nod_params_Sophia ] ] # large-amplitude slow nod; realistic transits

three_freqs=np.asarray([300,900,1500])*u.MHz
...
for experimental_setup in experimental_setups:
    ...
    az_scatter, za_scatter = draw_tracks(el_delta=0.5, tres=0.01,
                                         rots=np.atleast_1d((0.,)), # ok to hard-code lack of rotation in this script because none of the hypotheticals involve rotation
    ...
    fig= plot_omniscient_sky(az_scatter, za_scatter, CasA,
                             freqs=three_freqs)
    fig.savefig("{}/omniscient_sky_{}.png".format(outdir,name),
                dpi=dpiuse)
    print("generated omniscient sky figure")
